...
class LineSegmentDetectorImpl : public LineSegmentDetectorNew
{
public:
 ...
 */
    LineSegmentDetectorImpl(int _refine = LSD_REFINE_STD, float _min_length = 10, double _scale = 0.8,
    ...
    double _log_eps, double _density_th, int _n_bins)
{
    return makePtr<LineSegmentDetectorImpl>(
            _refine, _min_length, _scale, _sigma_scale, _quant, _ang_th,
            _log_eps, _density_th, _n_bins);
}

/////////////////////////////////////////////////////////////////////////////////////////

LineSegmentDetectorImpl::LineSegmentDetectorImpl(int _refine, float _min_length, double _scale, double _sigma_scale, double _quant,
        double _ang_th, double _log_eps, double _density_th, int _n_bins)
        :SCALE(_scale), doRefine(_refine), SIGMA_SCALE(_sigma_scale), QUANT(_quant),
        ANG_TH(_ang_th), LOG_EPS(_log_eps), DENSITY_TH(_density_th), N_BINS(_n_bins),MIN_LENGTH(_min_length)
{
...
void LineSegmentDetectorImpl::detect(InputArray _image, OutputArray _lines,
                InputArray _mask, OutputArray _width, OutputArray _prec, OutputArray _nfa)
{
    image = _image.getMat();
    ...
    if(!mask.empty())
    {
        CV_Assert(mask.size()==image.size() && mask.type()==CV_8UC1);
    }
    ...
    std::vector<double>& nfas)
{
    ...
    if(SCALE != 1)
    {
        ...
        if(!mask.empty()) resize(mask,mask,Size(),SCALE,SCALE);
        ll_angle(rho, N_BINS);
    }
    else
    {
    ...
    for(size_t i = 0, list_size = list.size(); i < list_size; ++i)
    {
        ...
        if((used.at<uchar>(point) == NOTUSED) && (angles.at<double>(point) != NOTDEF))
        {
            ...
            if(hypotf(rec.x1-rec.x2,rec.y1-rec.y2)<MIN_LENGTH){ continue; }
                                   ...
                                   const unsigned int& n_bins)
{
    ...
    for(int y = 0; y < img_height - 1; ++y)
    {
        ...
        const uchar* mask_row = use_mask ? mask.ptr<uchar>(y) : nullptr;
...
void LineSegmentDetectorImpl::drawSegments(InputOutputArray _image, InputArray lines)
{
    ...
    for(int i = 0; i < N; ++i)
    {
        ...
        Point2f b(v[0], v[1]);
        Point2f e(v[2], v[3]);
        line(_image.getMatRef(), b, e, Scalar(0, 0, 255), 2);
    }
}
