...
void processFile(const char* fimu, const char* fgt, int start_index = 0, double time_gap = 1) {
  ...
  for (size_t imu_idx = start_index; imu_idx < imu_datas.size(); imu_idx++) {
    ...
    if (dt > 0) {
      vs::imuPredict<double>(dt, gyro - estimate_state.bg, acc - estimate_state.ba, gravity, estimate_state.q,
                             estimate_state.p, estimate_state.v);
...
void processFile2(const char* fimu, const char* fgt, int start_index = 0, double time_gap = 1) {
  auto imu_datas = vs::loadFileData(fimu, ',');
  auto gt_datas = vs::loadFileData(fgt, ',');
  if (imu_datas.empty() || gt_datas.empty()) return;
  std::vector<State> gt_state_list = datas2states(gt_datas);
  ...
  double last_reset_ts = 0;
  vs::ImuPredictor<double> predictor;
  std::vector<State> estimate_state_list;
  int gt_idx = 0;
  Eigen::Vector3d gravity(0, 0, -9.81);
  for (size_t imu_idx = start_index; imu_idx < imu_datas.size(); imu_idx++) {
    ...
    if (need_reset) {
      last_reset_ts = gt_state.ts;
      predictor.updateState(gt_state.ts, gt_state.q, gt_state.p, gt_state.v, gt_state.ba, gt_state.bg, gravity);
    }
    predictor.updateImu(imu_ts, gyro, acc);
    State estimate_state;
    if (predictor.getLatestState(estimate_state.ts, estimate_state.q, estimate_state.p, estimate_state.v,
                                 estimate_state.ba, estimate_state.bg))
      estimate_state_list.push_back(estimate_state);
  }
...
int main(int argc, char** argv) {
  if (argc < 3) {
    printf("Usage: %s imu_data.csv gt_pose.csv [start_index] [time_gap]", vs::filename(vs::basename(__FILE__).c_str()).c_str());
    return -1;
  }
  int start_index = 0;
  double time_gap = 1;
  ...
  processFile2(argv[1], argv[2], start_index, time_gap);
}